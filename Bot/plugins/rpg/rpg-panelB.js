...
let handler = async (m, { conn, text, usedPrefix, isOwner }) => {
  ...
  if(aksi === 'toprpg'){
    ...
    let topLevel = users.filter(id => wdb.users[id].rpg).sort((a,b) => (wdb.users[b].rpg.level || 0) - (wdb.users[a].rpg.level || 0)).slice(0, 10)
    let topMoney = users.sort((a,b) => (wdb.money[b] || 0) - (wdb.money[a] || 0)).slice(0, 10)
    let topDiamond = users.filter(id => wdb.users[id].rpg).sort((a,b) => (wdb.users[b].rpg.diamond || 0) - (wdb.users[a].rpg.diamond || 0)).slice(0, 10)
  ...
  // 2. EDIT DATA - INI BUAT TURUNIN OWNER DARI TOP1
  if(!who) return m.reply('❌ Tag target dulu')
  let user = wdb.users[who]?.rpg
  if(!user) return m.reply('❌ Target belum punya data RPG', null, {mentions: [who]})
  ...
  if(aksi === 'daily'){
    let hadiah = 50000
    wdb.money[who] = (wdb.money[who] || 0) + hadiah
