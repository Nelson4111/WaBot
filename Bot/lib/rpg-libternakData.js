...
function generateHybrid(h1, h2, evo) {
  ...
  return {
    id, emoji: h1.emoji + h2.emoji, nama, hargaBibit: 0,
    hargaJual: Math.floor(baseJual * 1.5 * (evo + 1)),
    hasilTelur: `Telur ${nama}`, hasilDaging: `Daging ${nama}`, hasil: `Hasil ${nama}`,
    exp: Math.floor(baseExp * (evo + 1) * 1.2), tipe: 'all', evolusi: evo
  }
}

export function prosesKawin(id1, id2) {
  let hybridDatabase = getHybridDB()
  let h1 = hewanList[id1] || hybridDatabase[id1]
  let h2 = hewanList[id2] || hybridDatabase[id2]
  if(!h1 ||!h2) return null
  if(id1 === id2) return { hasil: id1, baru: false, data: h1 }
  let evoBaru = Math.min(Math.max(h1.evolusi, h2.evolusi) + 1, 7)
  let idHybrid = [id1, id2, evoBaru].sort().join('_')
  let baru = false
...
export function getHewan(id) {
  let hybridDatabase = getHybridDB()
  return hewanList[id] || hybridDatabase[id] || null
}