import axios from 'axios'
import FormData from 'form-data'
import { fileTypeFromBuffer } from 'file-type'

const toSmallNum = (str) => {
  const map = { '0': '𝟶', '1': '𝟷', '2': '𝟸', '3': '𝟹', '4': '𝟺', '5': '𝟻', '6': '𝟼', '7': '𝟽', '8': '𝟾', '9': '𝟿' }
  return String(str || '').replace(/[0-9]/g, d => map[d] || d)
}

async function uploadToTermai(buffer) {
  const fileType = await fileTypeFromBuffer(buffer) || { ext: 'jpg', mime: 'image/jpeg' }
  const form = new FormData()
  form.append('file', buffer, `upload.${fileType.ext}`)

  const res = await axios.post('https://c.termai.cc/api/upload', form, {
    headers: form.getHeaders(),
    timeout: 20000
  })
  if (res.data?.status && res.data?.url) return res.data.url
  if (res.data?.url) return res.data.url
  throw new Error('Termai upload failed')
}

async function uploadToCatbox(buffer) {
  const fileType = await fileTypeFromBuffer(buffer) || { ext: 'jpg', mime: 'image/jpeg' }
  const form = new FormData()
  form.append('reqtype', 'fileupload')
  form.append('fileToUpload', buffer, `upload.${fileType.ext}`)

  const res = await axios.post('https://catbox.moe/user/api.php', form, {
    headers: form.getHeaders(),
    timeout: 20000
  })
  if (typeof res.data === 'string' && res.data.startsWith('http')) return res.data.trim()
  throw new Error('Catbox upload failed')
}

async function uploadImage(buffer) {
  try {
    return await uploadToTermai(buffer)
  } catch (e1) {
    return await uploadToCatbox(buffer)
  }
}

let handler = async (m, { conn, usedPrefix, command }) => {
  const q = m.quoted ? m.quoted : m
  const mime = (q.msg || q).mimetype || ''

  if (!mime || !mime.startsWith('image/')) {
    return m.reply(`*╭  〔 ⚠ ᴘ ᴇ ʀ ɪ ɴ ɢ ᴀ ᴛ ᴀ ɴ 〕*
> Format input salah! Kirim atau reply gambar yang ingin dicek:
> › *${usedPrefix + command}* (reply gambar)
*╰───────────────*`)
  }

  await conn.sendMessage(m.chat, { react: { text: '⏳', key: m.key } })

  try {
    const imgBuffer = await q.download()
    if (!imgBuffer) throw new Error('Gagal mengunduh gambar sumber.')

    const uploadedUrl = await uploadImage(imgBuffer)

    let data = null
    try {
      const res = await axios.get(`https://api.ryzumi.net/api/tool/nsfw-checker?url=${encodeURIComponent(uploadedUrl)}`, {
        timeout: 20000,
        headers: { 'User-Agent': 'Mozilla/5.0' }
      })
      data = res.data
    } catch (e1) {
      const res2 = await axios.get(`https://api.ryzumi.net/api/tool/v2/nsfw-checker?url=${encodeURIComponent(uploadedUrl)}`, {
        timeout: 20000,
        headers: { 'User-Agent': 'Mozilla/5.0' }
      })
      data = res2.data
    }

    if (!data || data.error) {
      throw new Error(data?.error || 'Gagal memproses moderasi gambar.')
    }

    await conn.sendMessage(m.chat, { react: { text: '✅', key: m.key } })

    const isNsfw = data.is_nsfw || data.nsfw || false
    const score = data.score !== undefined ? `${Math.round(data.score * 100)}%` : '-'

    const caption = `*──  ୨୧ ✧ ɴꜱꜰᴡ ᴄʜᴇᴄᴋᴇʀ ✧ ୨୧  ──*

*╭  〔 🛡 ᴍ ᴏ ᴅ ᴇ ʀ ᴀ ꜱ ɪ  ᴋ ᴏ ɴ ᴛ ᴇ ɴ 〕*
*┆* ⟡ ꜱᴛᴀᴛᴜꜱ ɴꜱꜰᴡ : *${isNsfw ? 'Terdeteksi NSFW (18+) ⚠️' : 'Aman / SFW (Bersih) ✅'}*
*┆* ✧ ᴛɪɴɢᴋᴀᴛ     : *${toSmallNum(score)}*
*╰───────────────*

> _Pemeriksaan moderasi otomatis berbasis deteksi computer vision._`.trim()

    return m.reply(caption)
  } catch (err) {
    await conn.sendMessage(m.chat, { react: { text: '❌', key: m.key } })
    const msg = err.response?.data?.message || err.message
    return m.reply(`*╭  〔 ✕ ɢ ᴀ ɢ ᴀ ʟ 〕*
> Gagal memeriksa moderasi NSFW: ${msg}
*╰───────────────*`)
  }
}

handler.help = ['ceknsfw', 'nsfwcheck']
handler.tags = ['tools']
handler.command = /^(ceknsfw|nsfwcheck|nsfw)$/i
handler.limit = true

export default handler
