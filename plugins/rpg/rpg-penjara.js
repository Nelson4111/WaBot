...
let handler = async (m, { conn, args, command, usedPrefix }) => {
    ...
    if(command === 'penjarain'){
        if(!handler.owner) return m.reply('❌ Khusus Owner')
...
handler.before = async function (m, { conn }) {
    ...
    let plugin = Object.values(global.plugins).find(p => p.tags && p.tags.includes('rpg') && p.command && new RegExp(p.command).test(command))
    if(plugin){
...
handler.owner = true // <--- ini kunci aksesnya. Sama kaya addmoney
handler.group = true
