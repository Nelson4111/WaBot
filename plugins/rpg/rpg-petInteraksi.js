...
if (action === 'feed') {
  ...
  if (user.pets.some(p => Date.now() - (p.lastFeed || 0) < cd)) return m.reply(
    ...
    `╰─━━━━━━━━━━━━━━─\n\n` +
    `> ↳ Tunggu 2 menit dulu sebelum memberi makan lagi.`
  )

  let biayaMakan = 5000 * user.pets.filter(p => p.tipe !== 'robot').length

  if ((wdb.money[m.sender] || 0) < biayaMakan) return m.reply(
    `╭─❏「 🐾 AVELIA PET CENTER 」❏\n` +
    `│ ❌ *UANG TIDAK CUKUP*\n` +
    `╰─━━━━━━━━━━━━━━─\n\n` +
  ...
  teks += `> ↳ 💰 Biaya: -Rp ${biayaMakan.toLocaleString()}\n`
  teks += `> ↳ 📊 Status: +25 Exp | +20 Energy | +5 Happy\n`

  if(naik.length) {
    teks += `\n─━━━━━━━━━━━━━━─\n\n`
    teks += `🎉 *LEVEL UP!*\n`
    teks += naik.map(n => `> ↳ ${pets[n].emoji} ${formatNama(n)}`).join('\n')
  }
...
if (action === 'train') {
  ...
  if(getDebuff(user.pets[0]) < 1) {
    teks += `> ↳ 💩 Debuff: Pet kotor! Exp -${(1-getDebuff(user.pets[0]))*100}%\n`
  }

  teks += `> ↳ 📊 Status: +60 Exp | -30 Energy | -5 Happy | +20 Dirty\n`
...
if (action === 'walk') {
  ...
  if (Date.now() - (user.pets[0].lastActivity || 0) < cd) return m.reply(
    `╭─❏「 🐾 AVELIA PET CENTER 」❏\n` +
    `│ ⏰ *BELUM BISA JALAN-JALAN*\n` +
    `╰─━━━━━━━━━━━━━━─\n\n` +
    `> ↳ Tunggu ${cd/1000} detik dulu sebelum jalan lagi.`
  )
  ...
  teks += `> ↳ 📊 Status: +30 Exp | -20 Energy | +5 Happy | +15 Dirty\n`
...
if (action === 'play') {
  ...
  if (Date.now() - (user.pets[0].lastActivity || 0) < cd) return m.reply(
    ...
    `│ ⏰ *BELUM BISA BERMAIN*\n` +
    `╰─━━━━━━━━━━━━━━─\n\n` +
    `> ↳ Tunggu ${cd/1000} detik dulu sebelum bermain lagi.`
  )
  ...
  teks += `> ↳ 📊 Status: +30 Exp | -20 Energy | +10 Happy | +10 Dirty\n`
...
if (action === 'rest') {
  ...
  if (Date.now() - (user.pets[0].lastRest || 0) < cd) return m.reply(
    ...
    `│ ⏰ *BARU SAJA ISTIRAHAT*\n` +
    `╰─━━━━━━━━━━━━━━─\n\n` +
    `> ↳ Baru istirahat. Tunggu ${Math.floor(cd/60000)} menit lagi`
  )
...
if (action === 'clean') {
  ...
  if(user.pets.every(p => (p.dirty || 0) === 0)) return m.reply(
    `╭─❏「 🐾 AVELIA PET CENTER 」❏\n` +
    `│ ✨ *PET SUDAH BERSIH*\n` +
    `╰─━━━━━━━━━━━━━━─\n\n` +
  ...
  user.pets.forEach(p => {
    p.dirty = 0
    p.happy = Math.min(100, (p.happy || 50) + 10)
  })
...
if (action === 'battle') {
  ...
  if(!target) return m.reply(
    `╭─❏「 🐾 AVELIA PET CENTER 」❏\n` +
    `│ ❌ *TARGET BATTLE TIDAK ADA*\n` +
    `╰─━━━━━━━━━━━━━━─\n\n` +
    `> ↳ Tag atau reply orang yang mau kamu ajak battle!\n` +
  ...
  if((myPet.energy || 100) < 20) return m.reply(
    ...
    `│ ❌ *ENERGY PET RENDAH*\n` +
    `╰─━━━━━━━━━━━━━━─\n\n` +
    `> ↳ Pet ${formatNama(myPet.tipe)} kamu energy < 20%`
  )

  if((enemyPet.energy || 100) < 20) return m.reply(
    `╭─❏「 🐾 AVELIA PET CENTER 」❏\n` +
    `│ ❌ *ENERGY PET TARGET RENDAH*\n` +
    `╰─━━━━━━━━━━━━━━─\n\n` +
    `> ↳ Pet ${formatNama(enemyPet.tipe)} target energy < 20%`
  )
  ...
  if(taruhan > 0){
    if(taruhan < 1000) return m.reply(
      `╭─❏「 🐾 AVELIA PET CENTER 」❏\n` +
      `│ ❌ *TARUHAN TERLALU KECIL*\n` +
      `╰─━━━━━━━━━━━━━━─\n\n` +
      `> ↳ Minimal taruhan Rp 1000`
    )
  ...
  cap += `> ↳ ${pets[myPet.tipe].emoji} ${formatNama(myPet.tipe)} Lv.${myPet.level}\n`
  cap += `> ↳ Power: ${powerA}\n\n`

  cap += `⚔️ *VS*\n\n`

  cap += `👤 *${'@' + target.split('@')[0]}*\n`
  cap += `> ↳ ${pets[enemyPet.tipe].emoji} ${formatNama(enemyPet.tipe)} Lv.${enemyPet.level}\n`
  cap += `> ↳ Power: ${powerB}\n`

  if(taruhan > 0) {
    cap += `\n💰 *TARUHAN*\n`
    cap += `> ↳ Rp ${taruhan.toLocaleString()}\n`
  }

  if(powerA > powerB){
    if(taruhan > 0){
      wdb.money[m.sender] += taruhan
      wdb.money[target] -= taruhan
    }

    user.pets[idxA].exp += 40
    userTarget.pets[idxB].exp += 10

    if(user.pets[idxA].exp >= 100){
      user.pets[idxA].level++
      user.pets[idxA].exp = 0
      cap += `\n🎉 *${formatNama(myPet.tipe)} LEVEL UP!*\n`
    }
...
handler.tags = ['rpg']
handler.command = ['petinteraksi']
handler.alias = ['petinteraksi']
