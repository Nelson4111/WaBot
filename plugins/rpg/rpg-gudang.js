...
let handler = async (m, { conn, usedPrefix }) => {
  ...
  // GABUNGIN SEMUA INVENTORY JADI 4 KATEGORI
  let grouped = {
  ...
  for(let item in user.inventory){
    if(user.inventory[item] > 0){
      ...
      totalJenis++
      let emoji = bibit[item] || '📦'
      grouped['HASIL PANEN'].list.push({nama: item, emoji, jml: user.inventory[item]})
    }
  }
  // 2. IKAN
  ...
  if(totalJenis === 0)
    return m.reply(`┌───❏「 📦 GUDANG KOSONG 」❏\n│\n│ Tanam, mancing, tambang, adventure, atau masak dulu\n└───────────────────`)

  // URUTIN PER KATEGORI DARI PALING BANYAK
  ...
  let urutan = ['HASIL PANEN','IKAN','MATERIAL','MASAKAN']
  for(let t of urutan){
