...
let handler = async (m, { conn, command }) => {
    ...
    try {
        ...
        let apiUrl = `https://api-faa.my.id/faa/tochibi?url=${encodeURIComponent(fotoUrl)}`

        await conn.sendFile(m.chat, apiUrl, 'chibi.jpg', `✅ *To Chibi Berhasil*`, m)
        await conn.sendMessage(m.chat, { react: { text: '✅', key: m.key } })
    } catch (e) {
        console.error(e)
        await conn.sendMessage(m.chat, { react: { text: '❌', key: m.key } })
        m.reply('❌ Gagal memproses gambar menjadi Chibi.')
    }
}

handler.help = ['tochibi']
handler.tags = ['maker']
handler.command = /^(tochibi)$/i
handler.limit = true
