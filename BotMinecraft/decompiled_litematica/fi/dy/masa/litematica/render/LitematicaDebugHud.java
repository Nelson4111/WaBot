 ...
 *  fi.dy.masa.malilib.util.game.DebugHudUtils
...
implements DebugScreenEntry {
    public static final Identifier LITEMATICA_DEBUG = Identifier.fromNamespaceAndPath((String)"litematica", (String)"litematica_renderer");
    public static final Identifier SECTION_ID = Identifier.withDefaultNamespace((String)"litematica");
    ...
    public void checkConfig() {
        ...
        if (this.getMode() == DebugHudMode.VANILLA) {
            DebugHudUtils.register((Identifier)LITEMATICA_DEBUG, (DebugScreenEntry)INSTANCE);
        } else if (this.getMode() != DebugHudMode.VANILLA) {
            DebugHudUtils.unregister((Identifier)LITEMATICA_DEBUG);
        }
    }
    ...
    private @NonNull String getWorldDebug(WorldSchematic worldSchematic) {
        String str = Reference.DEBUG_MODE ? String.format("[%s] // TE: %02d C: %02d, CT: %02d, CV: %02d", worldSchematic.getEntityDebug(), worldSchematic.getChunkSource().getTileEntityCount(), worldSchematic.getChunkSource().getLoadedChunksCount(), DataManager.getSchematicPlacementManager().getTouchedChunksCount(), DataManager.getSchematicPlacementManager().getLastVisibleChunksCount()) : String.format("E: %02d TE: %02d C: %02d, CT: %02d, CV: %02d", worldSchematic.getRegularEntityCount(), worldSchematic.getChunkSource().getTileEntityCount(), worldSchematic.getChunkSource().getLoadedChunksCount(), DataManager.getSchematicPlacementManager().getTouchedChunksCount(), DataManager.getSchematicPlacementManager().getLastVisibleChunksCount());
        return str;
    }
}
