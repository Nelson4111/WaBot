...
implements AutoCloseable {
    private final ConcurrentHashMap<ChunkSectionLayer, MeshData> blockMeshData = new ConcurrentHashMap(ByteBufferBuilderCache.BLOCK_LAYERS.size(), 0.9f, 1);
    private final ConcurrentHashMap<OverlayRenderType, MeshData> overlayMeshData = new ConcurrentHashMap(ByteBufferBuilderCache.TYPES.size(), 0.9f, 1);
    ...
    protected boolean hasMeshData(OverlayRenderType type) {
        return this.overlayMeshData.containsKey((Object)type);
    }
    ...
    protected MeshData getMeshDataOrNull(OverlayRenderType type) {
        return this.overlayMeshData.get((Object)type);
    }
