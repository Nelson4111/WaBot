...
IInfoHudRenderer {
    ...
    protected void updateInfoHudLinesPendingChunks(Collection<ChunkPos> pendingChunks) {
        ...
        if (!pendingChunks.isEmpty() && this.mc.player != null) {
            ArrayList<ChunkPos> list = new ArrayList<ChunkPos>(pendingChunks);
            PositionUtils.CHUNK_POS_COMPARATOR.setReferencePosition(BlockPos.containing((Position)this.mc.player.position()));
            ...
            for (int i = 0; i < maxLines; ++i) {
                ChunkPos pos = (ChunkPos)list.get(i);
