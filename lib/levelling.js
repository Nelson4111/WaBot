...
export async function checkLevelUp(m, conn) {
  ...
  if (chat && (typeof chat.autolevelupLevel !== 'number' || isNaN(chat.autolevelupLevel))) chat.autolevelupLevel = 0
  if (chat && chat.autolevelup === false) return false
  ...
  if (canLevelUp(user.level, user.exp, multiplier)) {
    ...
    if (chat && Number(chat.autolevelupLevel) > 0 && newLevel < Number(chat.autolevelupLevel)) return false
