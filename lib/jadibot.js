...
const jadibotRetryCounterMap = new Map()
const jadibotRetryCounterCache = {
  get: (key) => {
    const entry = jadibotRetryCounterMap.get(key)
    if (!entry) return undefined
    if (Date.now() - entry.time > 15 * 60 * 1000) {
      jadibotRetryCounterMap.delete(key)
      return undefined
    }
    return entry.value
  },
  set: (key, value) => {
    jadibotRetryCounterMap.set(key, { value, time: Date.now() })
  },
  del: (key) => {
    jadibotRetryCounterMap.delete(key)
  }
...
export async function resolveWaUser(conn, input, m = null) {
  ...
  const hasInternationalPlus = String(raw).trim().startsWith('+')
  ...
  } else if (!hasInternationalPlus && cleanDigits.startsWith('8') && cleanDigits.length >= 9 && cleanDigits.length <= 13) {
...
export async function deleteSessionAndNotify(phoneNumber, userJid, reason) {
  ...
  if (record) {
    ...
    if (record.heartbeatInterval) clearInterval(record.heartbeatInterval)
...
export async function stopJadibot(phoneNumber, notify = true, requesterJid = null) {
  ...
  if (record) {
    record.isStopping = true
    if (record.heartbeatInterval) clearInterval(record.heartbeatInterval)
    if (record.pairingTimeout) clearTimeout(record.pairingTimeout)
    try { record.sock?.ws?.close() } catch {}
    try { record.sock?.ev?.removeAllListeners() } catch {}
    global.jadibots.delete(cleanPhone)
  }
 ...
 * @param {boolean|object} options Opsi { isStartup: boolean, isReconnect: boolean } atau boolean isStartup
 */
export async function startJadibot(phoneNumber, userJid, m = null, options = {}) {
  const isStartup = typeof options === 'boolean' ? options : !!options.isStartup
  const isReconnect = typeof options === 'object' ? !!options.isReconnect : false
  ...
  // Periksa apakah sesi sebelumnya sudah terdaftar (registered)
  let isAlreadyRegistered = false
  if (existsSync(credsPath)) {
    try {
      const rawCreds = JSON.parse(readFileSync(credsPath, 'utf-8'))
      if (rawCreds && rawCreds.registered) {
        isAlreadyRegistered = true
      }
    } catch {}
  }

  // Jika ini permintaan pairing baru (bukan reconnect & bukan startup) dan sesi belum terdaftar,
  // bersihkan folder sesi agar tidak ada pre-key/noise key basi yang memicu 'couldn't link device'
  if (!isStartup && !isReconnect && !isAlreadyRegistered && existsSync(sessionDir)) {
    try {
      rmSync(sessionDir, { recursive: true, force: true })
      console.log(chalk.cyan(`[JADIBOT] Membersihkan sesi belum terdaftar sebelum pairing baru untuk +${cleanPhone}`))
    } catch {}
  }
  ...
  // Jika sudah ada record berjalan dan socketnya masih aktif, lewati agar tidak loop restart
  if (global.jadibots.has(cleanPhone)) {
    const existing = global.jadibots.get(cleanPhone)
    if (existing && existing.status === 'open' && existing.sock?.ws?.isOpen) {
      return {
        alreadyActive: true,
    ...
    if (existing.pairingTimeout) clearTimeout(existing.pairingTimeout)
  ...
  const connectionOptions = {
    ...
    msgRetryCounterCache: jadibotRetryCounterCache,
    ...
    getMessage: async (key) => {
      try {
        ...
        if (global.db?.data?.msgs?.[key.id]) {
          return global.db.data.msgs[key.id].message || undefined
        }
      } catch {}
      return undefined
    },
  ...
  sock.credsUpdate = async () => {
    try {
      await saveCreds()
    } catch (e) {
      console.error(`[JADIBOT] Gagal menyimpan kredensial +${cleanPhone}:`, e.message)
    }
  }
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      const isFirstConnect = record.status !== 'open'
      record.status = 'open'
      record.retryCount = 0
      sock.connectTime = Date.now()
      sock.isOnline = true
      ...
      if (isFirstConnect) {
        console.log(chalk.green(`✅ [JADIBOT] Sub-bot +${cleanPhone} berhasil terhubung (Online).`))
      }
    }
    ...
    if (connection === 'close') {
      ...
      record.status = 'close'
      sock.isOnline = false
      sock.lastDisconnectTime = Date.now()
      ...
      // Jika sesi belum terdaftar dan BUKAN restartRequired (515), jangan lakukan reconnect agresif
      if (!isRegistered && !isRestartRequired) {
        console.log(chalk.yellow(`[JADIBOT] Koneksi pairing untuk +${cleanPhone} tertutup (${errMessage} - status: ${statusCode || 'unknown'}).`))
        return
      }

      if (prevStatus === 'open') {
        console.log(chalk.yellow(`⚠️ [JADIBOT] Sub-bot +${cleanPhone} terputus (${errMessage} - status: ${statusCode || 'none'}). Menghubungkan ulang...`))
      }

      // Reconnect: jika 515 (restart required saat linking selesai), reconnect segera (1 detik)
      record.retryCount = (record.retryCount || 0) + 1
      const delay = isRestartRequired ? 1000 : Math.min(60000, 3000 + record.retryCount * 4000)
      setTimeout(async () => {
        if (record.isStopping) return
        try {
          await startJadibot(cleanPhone, metadata.userJid, null, { isReconnect: true })
        } catch (e) {
          console.error(`[JADIBOT] Reconnect error for +${cleanPhone}:`, e.message)
        }
      }, delay)
    }
  })

  // Jika sesi belum terdaftar
  if (!sock.authState.creds.registered) {
    ...
    if (isReconnect) {
      // Sedang reconnect untuk menyelesaikan proses login / pairing (misal setelah status 515)
      // Jangan meminta pairing code baru, biarkan socket menyelesaikan handshake
      return {
        success: true,
        pendingPairing: true,
        phoneNumber: cleanPhone
    ...
    try {
      ...
      // Set timeout pembatalan otomatis jika user tidak memasukkan kode dalam 180 detik (3 menit)
      if (record.pairingTimeout) clearTimeout(record.pairingTimeout)
      record.pairingTimeout = setTimeout(() => {
        const cur = global.jadibots.get(cleanPhone)
        if (cur && cur.status !== 'open' && !cur.sock?.authState?.creds?.registered) {
          console.log(chalk.yellow(`[JADIBOT] Pairing code timeout untuk +${cleanPhone} (180 detik). Menghapus sesi.`))
...
export async function initAllJadibots(mainConn) {
  ...
  try {
    ...
    for (const phone of folders) {
      // PENTING: Jika sub-bot ini SUDAH online dan aktif, LEWATI! Jangan reconnect ulang!
      const existing = global.jadibots?.get(phone)
      if (existing && existing.status === 'open' && existing.sock?.ws?.isOpen) {
        continue
      }
      ...
      // Jika tidak ada creds.json atau registered: false, berarti belum pernah berhasil pairing, hapus foldernya
      let isRegistered = false
      if (existsSync(credsPath)) {
        try {
          const rawCreds = JSON.parse(readFileSync(credsPath, 'utf-8'))
          if (rawCreds && rawCreds.registered) isRegistered = true
        } catch {}
      }
      ...
      if (!isRegistered) {
        try {
          rmSync(sessionDir, { recursive: true, force: true })
          console.log(chalk.gray(`[JADIBOT] Membersihkan sesi belum terdaftar/batal saat startup: ${phone}`))
        } catch {}
        continue
      }
      ...
      try {
        ...
        await startJadibot(phone, userJid, null, { isStartup: true })
