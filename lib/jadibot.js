...
export async function startJadibot(phoneNumber, userJid, m = null, options = {}) {
  ...
  if (global.jadibots.has(cleanPhone)) {
    ...
    if (existing.status === 'open' && !isStartup && !isReconnect) {
      return {
        alreadyActive: true,
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      ...
      // Heartbeat ping agar socket tidak zombie dan tetap responsif
      if (record.heartbeatInterval) clearInterval(record.heartbeatInterval)
      record.heartbeatInterval = setInterval(async () => {
        if (record.status !== 'open' || record.isStopping) return
        try {
          if (sock.ws && sock.ws.readyState !== 1) {
            sock.end(new Error('Socket zombie detected'))
          } else if (sock.ws && typeof sock.ws.ping === 'function') {
            sock.ws.ping()
          }
        } catch {}
      }, 30000)
    ...
    if (connection === 'close') {
      ...
      if (!isRegistered && !isRestartRequired) {
        console.log(chalk.yellow(`[JADIBOT] Koneksi pairing untuk +${cleanPhone} tertutup sementara (status: ${statusCode || 'unknown'}).`))
        return
      }
