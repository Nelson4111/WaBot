...
export async function startJadibot(phoneNumber, userJid, m = null, isRestart = false) {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      // Reconnect tanpa menghapus sesi (jangan hapus sesi saat gangguan koneksi sementara)
      record.retryCount = (record.retryCount || 0) + 1
      const delay = Math.min(60000, 3000 + record.retryCount * 4000)
      setTimeout(async () => {
        if (record.isStopping) return
        try {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (cur && cur.status !== 'open' && !cur.sock?.authState?.creds?.registered) {
          // Timeout pairing 2 menit untuk cleanPhone, bersihkan
