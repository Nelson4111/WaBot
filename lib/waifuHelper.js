...
export async function searchMALCharacter(q) {
  ...
  const fallbackCharacters = {
    rem: { id: 34785, nama: 'Rem', image: null },
    'rem re zero': { id: 34785, nama: 'Rem', image: null }
  }
  const fallback = fallbackCharacters[query.toLowerCase()] || (isId && query === '34785' ? fallbackCharacters.rem : null)

  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      // 1. Ambil respons penuh dari Axios
      const response = await axios.get(url, {
        timeout: 4500,
        headers: { Accept: 'application/json', 'User-Agent': 'WaBot/1.0' }
      })
      
      // 2. Ekstrak body JSON dari Jikan API
      const jikanBody = response.data
      
      // 3. Sesuaikan ekstraksi berdasarkan endpoint yang dipanggil
      const c = isId ? jikanBody?.data : jikanBody?.data?.[0]
      
      // 4. Validasi objek karakter
      if (!c || !c.mal_id || !c.name) return null

      return {
        id: c.mal_id,
        nama: c.name,
        image: c.images?.jpg?.image_url || c.images?.webp?.image_url || null
      }
    } catch (error) {
      const status = error?.response?.status
      
      // Hentikan loop jika sudah percobaan terakhir ATAU error bukan masalah jaringan/rate-limit
      if (attempt === 1 || (status && ![408, 425, 429, 500, 502, 503, 504].includes(status))) {
        return fallback
      }
      
      // Jikan memiliki rate limit ketat. Jika terkena 429, beri jeda lebih lama (Exponential Backoff)
