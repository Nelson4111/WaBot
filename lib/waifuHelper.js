...
export async function searchMALCharacter(q) {
  ...
  // 1. Coba AniList GraphQL API (Cepat, stabil, tanpa batas scraper)
  try {
    const query = isId
      ? `
        query ($id: Int) {
    ...
    const variables = isId ? { id: parseInt(cleanQ) } : { search: cleanQ }
    ...
    if (char) {
      return {
        id: char.id,
        nama: char.name?.full || char.name?.native || 'Unknown Character',
  ...
  // 2. Fallback ke Jikan Moe API (MyAnimeList)
  try {
    const url = isId
      ? `https://api.jikan.moe/v4/characters/${encodeURIComponent(cleanQ)}`
      : `https://api.jikan.moe/v4/characters?q=${encodeURIComponent(cleanQ)}&limit=1`
    ...
    const c = isId ? data.data : data.data?.[0]
    if (c) {
      return {
        id: c.mal_id,
        nama: c.name,
    ...
    console.warn('[Jikan MAL Character Search Failed]:', e.message)
  }
