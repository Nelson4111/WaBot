 ...
 */
const formatStatus = (icon, title, targetOrMsg, ...extraArgs) => {
  let isTarget = targetOrMsg && typeof targetOrMsg.reply === 'function'
  let parts = isTarget ? extraArgs : [targetOrMsg, ...extraArgs].filter(Boolean)
  let lines = []
  for (const part of parts) {
    if (Array.isArray(part)) {
      lines.push(...part.map(p => `⟡ ${p}`))
  ...
  if (isTarget) {
    return targetOrMsg.reply(card)
  }
  return card
}

export const status = {
  wait: (targetOrMsg = 'Sedang memproses permintaanmu...', ...extra) => formatStatus('⧗', 'ᴍ ᴏ ʜ ᴏ ɴ  ᴛ ᴜ ɴ ɢ ɢ ᴜ', targetOrMsg, ...extra),
  success: (targetOrMsg = 'Operasi berhasil dilakukan!', ...extra) => formatStatus('✦', 'ʙ ᴇ ʀ ʜ ᴀ ꜱ ɪ ʟ', targetOrMsg, ...extra),
  error: (targetOrMsg = 'Terjadi kesalahan sistem.', ...extra) => formatStatus('✕', 'ɢ ᴀ ɢ ᴀ ʟ', targetOrMsg, ...extra),
  warning: (targetOrMsg = 'Periksa kembali format input kamu.', ...extra) => formatStatus('⚠', 'ᴘ ᴇ ʀ ɪ ɴ ɢ ᴀ ᴛ ᴀ ɴ', targetOrMsg, ...extra),
  info: (targetOrMsg = 'Informasi sistem.', ...extra) => formatStatus('ℹ', 'ɪ ɴ ꜰ ᴏ ʀ ᴍ ᴀ ꜱ ɪ', targetOrMsg, ...extra)
}
