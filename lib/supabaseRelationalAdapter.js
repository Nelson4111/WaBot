...
class SupabaseRelationalAdapter {
    ...
    async write(data) {
        ...
        try {
            ...
            // 3. Format seluruh root keys lainnya ke bot_metadata
            const metaPayload = []
            ...
            // 4. Eksekusi batch upsert secara paralel
            await Promise.all([
                this._batchUpsert('users', usersPayload),
                this._batchUpsert('chats', chatsPayload),
                this._batchUpsert('bot_metadata', metaPayload)
            ])
            ...
            // Sesuai permintaan user: tidak ada cadangan lokal, biarkan error tercatat
            throw err
        } finally {
            this._isWriting = false
        }
