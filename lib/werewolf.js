...
const buildRoleCodeMap = (room, player) => {
    ...
    if (player.role === "werewolf") {
        addTargetEntries(aliveTargets.slice(0, 4), "kill");
        ["Diam", "Tunggu", "Bukan malam ini"].forEach((label) => {
            addEntry({
                type: "skip",
                label
            });
        ...
        addTargetEntries(aliveTargets.slice(0, 4), "check");
        ["Diam", "Tunggu", "Bukan malam ini"].forEach((label) => {
            addEntry({
                type: "skip",
                label
        ...
        addTargetEntries(aliveTargets.slice(0, 4), "guard");
        ["Diam", "Tunggu", "Bukan malam ini"].forEach((label) => {
            addEntry({
                type: "skip",
                label
        ...
        addTargetEntries(aliveTargets.slice(0, 4), "poison");
        const reviveTargets = room.player.filter((p) => !p.isDummy && p.isdead && p.id !== player.id).slice(0, 4);
        ...
        ["Diam", "Tunggu", "Nggak pakai racun"].forEach((label) => {
            addEntry({
                type: "skip",
                label
            });
        ...
        if (player.isdead) {
            addTargetEntries(aliveTargets.slice(0, 4), "hunter");
        }
        ["Diam", "Tunggu", "Bukan malam ini"].forEach((label) => {
            addEntry({
                type: "skip",
...
const notifyDeadPlayers = async (conn, room) => {
    ...
    for (const player of room.player) {
        ...
        await conn.sendMessage(player.id, {
            text: `⚠️ Kamu telah mati di permainan Werewolf. Kamu tidak diperbolehkan mengetik aksi lagi agar permainan tetap berjalan lancar.`
        }).catch(() => {});
    }
};
    ...
    if (getWinner(x.room, data).status != null || x.win != null) {
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                ...
                if (x.player[i].isdead != true) {
                    textnya = `Kamu adalah Penyihir. Pilih aksi malam ini:\n*LIST PLAYER*:${tok1}\n\nKetik *.wwpc poison nomor* untuk meracuni atau *.wwpc revive nomor* untuk menghidupkan pemain yang mati.`;
            ...
            } else if (x.player[i].role === "hunter" && x.player[i].canShoot === true) {
                textnya = `Kamu adalah Hunter dan telah mati. Pilih satu pemain untuk ditembak:\n*LIST PLAYER*:${tok1}\n\nKetik *.wwpc hunter nomor*`;
