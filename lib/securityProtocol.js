...
export async function pruneSecurityUnverified(chatId, conn) {
  const metadata = typeof conn?.groupMetadata === 'function'
    ? await conn.groupMetadata(chatId).catch(() => null)
    : null
  if (!metadata?.participants) return 0

  const members = new Set((metadata.participants || [])
    .map(participant => securityJid(participant.id || participant.jid || participant.lid, conn))
    .filter(Boolean))
  const state = getSecurityState(chatId)
  const previousCount = state.unverified.length
  state.unverified = state.unverified.filter(jid => members.has(jid))
  const removedCount = previousCount - state.unverified.length
  if (removedCount > 0) await saveDB()
  return removedCount
}