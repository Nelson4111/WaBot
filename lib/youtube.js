...
async function downloadYouTubeMedia(videoUrl, format = 'mp3', quality = '128') {
  // 1. Primary: Ryzumi Fast YouTube Downloader
  try {
    const ep = format === 'mp3' ? 'ytmp3' : 'ytmp4'
    let ryzumiApi = `https://api.ryzumi.net/api/downloader/${ep}?url=${encodeURIComponent(videoUrl)}`
    if (format === 'mp4' && quality) {
      ryzumiApi += `&quality=${encodeURIComponent(quality)}`
    }

    const res = await axios.get(ryzumiApi, {
      timeout: 25000,
      headers: { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' }
    })

    const d = res.data
    const dlUrl = d?.url || d?.download || d?.link
    if (dlUrl) {
      ...
      const title = (d.title || 'YouTube Media').replace(/[<>:"/\\|?*\x00-\x1F]/g, '').trim()
      return {
    ...
    console.error('[Ryzumi YouTube DL Error, falling back]:', eRyzumi.message)
  }

  // 2. Fallback: cnv.cx converter
  const idMatch = videoUrl.match(/(?:youtu\.be\/|v=|\/v\/|\/embed\/|\/shorts\/)([a-zA-Z0-9_-]{11})/)
  const id = idMatch ? idMatch[1] : ''
  if (!id) throw new Error('ID video YouTube tidak valid.')

  const keyRes = await axios.get(`https://cnv.cx/v2/sanity/key?id=${id}`, {
    headers: { ...cnvHeaders, 'Content-Type': 'application/x-www-form-urlencoded' },
    timeout: 10000
  })
  const key = keyRes.data?.key
  if (!key) throw new Error('Gagal mendapatkan kunci converter cnv.cx.')

  const convRes = await axios.post('https://cnv.cx/v2/converter',
    new URLSearchParams({
      link: videoUrl,
      format: format === 'mp4' ? 'mp4' : 'mp3',
      audioBitrate: format === 'mp3' ? (quality || '128') : '128',
      videoQuality: format === 'mp4' ? (quality || '720') : '720',
      filenameStyle: 'pretty',
      vCodec: 'h264'
    }),
    { headers: { ...cnvHeaders, 'Content-Type': 'application/x-www-form-urlencoded', key }, timeout: 15000 }
  )

  let job = convRes.data
  let dlUrl = null
  let filename = job.filename || (format === 'mp3' ? 'audio.mp3' : 'video.mp4')
  ...
  if (job.status === 'tunnel' && job.url) {
    ...
    for (let i = 0; i < 20; i++) {
      await sleep(1500)
      const st = await axios.get(`https://cnv.cx/v2/status/${job.jobId}`, { headers: cnvHeaders, timeout: 10000 })
      if (st.data?.status === 'completed' && st.data?.url) {
        dlUrl = st.data.url
        filename = st.data.filename || filename
        break
      }
    }
  }

  if (!dlUrl) throw new Error('Converter YouTube timeout atau gagal memproses.')

  const resp = await axios.get(dlUrl, {
    responseType: 'arraybuffer',
    timeout: 60000,
    headers: {
      'User-Agent': cnvHeaders['User-Agent'],
  ...
  return {
    buffer: Buffer.from(resp.data),
    filename,
    title: filename.replace(/\.(mp3|mp4)$/i, '')
  }
}

export { youtubedl, youtubeSearch, downloadYouTubeMedia }