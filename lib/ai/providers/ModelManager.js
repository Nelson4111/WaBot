...
export class ModelManager {
    ...
    initProviders() {
        // 0. Official Hermes Agent Bridge (Phase A POC feature-flagged)
        if (process.env.HERMES_ENABLED === 'true') {
            const hermesBridge = new HermesBridge();
            this.providers.push(hermesBridge);
            console.log('[ModelManager] HERMES_ENABLED=true: Registered official HermesBridge as primary AI agent orchestrator.');
        }
        ...
        // Daftar model Groq aktif dan terkini (tercepat & berkualitas tinggi)
        const models = [
            'openai/gpt-oss-120b',
            'openai/gpt-oss-20b',
            'groq/compound-mini',
    ...
    async generate(messages, maxTokens = 300, options = {}) {
        let lastError = null;
        for (const provider of this.providers) {
            try {
                const response = await provider.generateResponse(messages, maxTokens, options);
