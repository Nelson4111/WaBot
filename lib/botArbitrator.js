    ...
    isAnyBot(jid) {
        ...
        if (global.jadibots && typeof global.jadibots.entries === 'function') {
            for (const [phone, record] of global.jadibots.entries()) {
                ...
                const botSocketId = record.sock?.user?.id || record.sock?.user?.jid || ''
                const botSocketNum = String(botSocketId).split('@')[0].split(':')[0].replace(/\D/g, '')
                if (botSocketNum && cleanNum === botSocketNum) return true
            }
        }
