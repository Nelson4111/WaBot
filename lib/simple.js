...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    conn.sendMessage = async function (jid, content, options = {}) {
        options = options || {}
        if (jid && typeof jid === 'string' && jid.endsWith('@g.us') && !options.cachedGroupMetadata) {
            options.cachedGroupMetadata = resolveGroupMetadataCached
        }
            ...
            async value(jid, text = '', footer = '', buffer, buttons = [], quoted, options = {}) {
                ...
                if (Array.isArray(buffer)) {
                    ...
                    try {
                        ...
                        buffer = type.data;
                        if (buffer) {
                            let media = await conn.prepareWAMessageMedia(
                                { [/video/.test(type.mime) ? 'video' : 'image']: buffer },
                                { upload: conn.waUploadToServer }
                            );
                            mediaHeader = {
                                title: "",
                ...
                if (!Array.isArray(buttons)) buttons = buttons ? [buttons] : [];
                if (buttons.length > 0 && !Array.isArray(buttons[0]) && typeof buttons[0] === 'string') {
                    buttons = [buttons];
                }
                if (!options) options = {};
                ...
                if (global.useButton) {
                                    ...
                                    nativeFlowMessage: {
                                        buttons: buttons.map(b => ({
                                            ...
                                            buttonParamsJson: JSON.stringify({
                                                display_text: Array.isArray(b) ? b[0] : (b.display_text || b.buttonText || b.text || ''),
                                                id: Array.isArray(b) ? b[1] : (b.id || b.buttonId || '')
                                            })
                                        }))
                                    }
                                }
                    ...
                    let btnText = buttons.map(b => `[ ${Array.isArray(b) ? b[0] : (b.display_text || b.buttonText || b.text)} ]`).join(' atau ');
                    let fallbackText = `${text}\n\n${footer}\n\nBalas pesan ini dengan mengetik perintah berikut:\n${btnText}`;
                    if (buffer) {
                        return conn.sendMessage(jid, { [/video/.test(type?.mime) ? 'video' : 'image']: buffer, caption: fallbackText.trim(), ...options }, { quoted, ...options });
                    }
                    return conn.sendMessage(jid, { text: fallbackText.trim(), ...options }, { quoted, ...options });
                }
            },
            enumerable: true
        },
        sendList: {
            async value(jid, title, text, footer, buttonText, sections, quoted, options = {}) {
                if (global.useButton) {
                    const msg = await generateWAMessageFromContent(jid, {
                        viewOnceMessage: {
                            message: {
                    ...
                    let listText = sections.map(s => {
                        let rows = s.rows.map(r => `  - ${r.title} : ${r.id}`).join('\n');
                        return `*${s.title}*\n${rows}`;
                    }).join('\n\n');
                    let fallbackText = `${title}\n${text}\n\n${footer}\n\nKetik salah satu ID (atau judul) dari daftar di bawah ini:\n\n${listText}`;
                    return conn.sendMessage(jid, { text: fallbackText.trim(), ...options }, { quoted, ...options });
                }
            },
            enumerable: true
...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        text: {
            get() {
                let msg = this.msg
                let unwrap = this.message
                while (unwrap && (unwrap.viewOnceMessage || unwrap.viewOnceMessageV2 || unwrap.ephemeralMessage || unwrap.documentWithCaptionMessage)) {
                    unwrap = unwrap.viewOnceMessage?.message || unwrap.viewOnceMessageV2?.message || unwrap.ephemeralMessage?.message || unwrap.documentWithCaptionMessage?.message;
                }
                const targetMsg = (unwrap && typeof unwrap === 'object') ? (unwrap.interactiveResponseMessage || unwrap[this.mtype] || unwrap) : msg

                const text = (typeof msg === 'string' ? msg : msg?.text) || msg?.caption || msg?.contentText || targetMsg?.body?.text || ''
                let interactiveId = ''
                const nativeParams = targetMsg?.nativeFlowResponseMessage?.paramsJson || msg?.nativeFlowResponseMessage?.paramsJson
                if (nativeParams) {
                    try { 
                        const parsed = JSON.parse(nativeParams)
                        interactiveId = parsed.id || parsed.rowId || parsed.selectedId || ''
                    } catch {}
                }
                
                return typeof this._text === 'string' ? this._text : '' || (typeof text === 'string' && text ? text : (
                    interactiveId ||
                    targetMsg?.selectedId ||
                    targetMsg?.selectedButtonId ||
                    targetMsg?.singleSelectReply?.selectedRowId ||
                    targetMsg?.selectedDisplayText ||
                    msg?.selectedId ||
