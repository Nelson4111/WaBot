...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    const msgQueues = {} // Antrean pesan per chat
    const delayQueue = (ms) => new Promise(resolve => setTimeout(resolve, ms))

    conn.sendMessage = async function (jid, content, options) {
        if (!msgQueues[jid]) msgQueues[jid] = Promise.resolve()

        const previousTask = msgQueues[jid]
        ...
        const currentTask = previousTask.then(async () => {
            ...
            try {
                return await originalSendMessage.apply(this, [jid, content, options])
            } finally {
                // Cooldown 1.5 detik per chat setelah mengirim pesan
                await delayQueue(1500)
            }
        })

        // Cegah antrean terhenti jika ada error
        msgQueues[jid] = currentTask.catch(() => {})
        ...
        sendButton: {
            async value(jid, text = '', footer = '', buttons = [], quoted, options = {}) {
                if (global.useButton) {
                                    ...
                                    nativeFlowMessage: {
                                        buttons: buttons.map(b => ({
                                            ...
                                            buttonParamsJson: JSON.stringify({
                                                display_text: b.display_text || b.buttonText || b.text,
                                                id: b.id || b.buttonId
                                            })
                                        }))
                                    }
                    ...
                    let btnText = buttons.map(b => `[ ${b.display_text || b.buttonText || b.text} ]`).join(' atau ');
