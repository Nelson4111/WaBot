...
END:VCARD`.trim()
   ...
   */
    ...
    */
        ...
        sendButton: {
            /**
             * send Button (Formatted as Clean Plain Chat Text)
             * @param {String} jid
             * @param {String} text
             * @param {String} footer
             * @param {Buffer} buffer
     ...
     */
        ...
        sendList: {
            /**
             * Send List formatted as Clean Chat Text
             */
            async value(jid, title = '', text = '', footer = '', buttonText = '', sections = [], quoted, options = {}) {
                let listText = (sections || []).map(s => {
                    let rows = (s.rows || []).map(r => `  • ${r.title} ${r.id ? `➔ *${r.id}*` : ''}${r.description ? ` (${r.description})` : ''}`).join('\n');
                    return `*${s.title}*\n${rows}`;
                }).join('\n\n');
                ...
                return conn.sendMessage(jid, { text: fullText, ...options }, { quoted, ...options });
            },
            enumerable: true
        },
        parseMention: {
