...
export async function sendDualGroupMessage(conn, groupJid, targetJid, playerMessage, spectatorText, options = {}) {
    ...
    await conn.authState.keys.transaction(async () => {
        ...
        try {
            ...
            fs.writeFileSync('./tmp/last_dual_debug.json', JSON.stringify({
...
/**
 * Mengirim 1 pesan grup tunggal yang berisi konten berbeda untuk setiap pemain aktif
 * (kartu peran rahasia + tombol aksi interaktif vertikal), sementara anggota grup lain
 * (penonton & pemain mati) menerima pesan teks biasa TANPA tombol sama sekali.
 * 
 * @param {Object} conn - Instance socket Baileys
 * @param {string} groupJid - JID grup (contoh: '120363xxx@g.us')
 * @param {Map<string, Object>|Object} playerMessagesMap - Map atau object { [playerJid]: playerMessageObj }
 * @param {string|Object} spectatorText - Teks / payload pesan untuk penonton & pemain mati (TANPA TOMBOL)
 * @param {Object} [options] - Opsi tambahan (messageId, contextInfo, dll)
 * @returns {Promise<Object>}
 */
export async function sendMultiParticipantGroupStanza(conn, groupJid, playerMessagesMap, spectatorText, options = {}) {
    if (!conn) throw new Error('Parameter conn diperlukan!')
    if (!groupJid) throw new Error('Parameter groupJid diperlukan!')

    const isGroup = groupJid.endsWith('@g.us')
    if (!isGroup) {
        const firstMsg = Object.values(playerMessagesMap)[0] || spectatorText
        return await conn.sendMessage(groupJid, typeof firstMsg === 'string' ? { text: firstMsg } : firstMsg, options)
    }
    ...
    try {
        ...
        console.warn('[MULTI-STANZA] Gagal ambil groupMetadata:', e?.message)
    }
    ...
    // Siapkan spectator message (HANYA teks polos, TANPA TOMBOL)
    let spectatorMsg
    if (typeof spectatorText === 'string') {
        spectatorMsg = {
            extendedTextMessage: {
        ...
        spectatorMsg = {
            extendedTextMessage: {
                text: 'Malam telah tiba di desa Werewolf...'
            }
        }
    }
    ...
    await conn.authState.keys.transaction(async () => {
        const playerEntries = playerMessagesMap instanceof Map 
            ? Array.from(playerMessagesMap.entries()) 
            : Object.entries(playerMessagesMap || {})

        const allDeviceJids = []
        const playerNodesPromises = []
        const registeredPlayerJids = new Set()

        // 2. Kumpulkan node pairwise untuk masing-masing pemain
        for (const [targetJid, rawMsg] of playerEntries) {
            if (!targetJid || !rawMsg) continue
            const cleanTargetJid = jidNormalizedUser(targetJid)
            const targetDigits = cleanTargetJid.split('@')[0].split(':')[0].replace(/\D/g, '')
            registeredPlayerJids.add(cleanTargetJid)

            const foundPlayer = participants.find(p => isMatchingUser(p, cleanTargetJid, targetDigits))
            const primaryJid = (isLidGroup && foundPlayer?.lid) ? foundPlayer.lid : (foundPlayer?.id || cleanTargetJid)
            const isTargetLid = primaryJid.endsWith('@lid')
            const domain = isTargetLid ? 'lid' : 's.whatsapp.net'

            const playerDevs = new Set()
            const { user: pu, device: pd } = jidDecode(primaryJid)
            if (pu) playerDevs.add(jidEncode(pu, domain, pd || 0))

            try {
                if (typeof conn.getUSyncDevices === 'function') {
                    const rawDevs = await conn.getUSyncDevices([primaryJid], false, false)
                    if (Array.isArray(rawDevs)) {
                        for (const { user, device } of rawDevs) {
                            playerDevs.add(jidEncode(user, domain, device || 0))
                        }
                    }
                }
            } catch (e) {}

            const devList = Array.from(playerDevs)
            allDeviceJids.push(...devList)

            const msgToEncode = rawMsg?.message || rawMsg
            playerNodesPromises.push(
                conn.createParticipantNodes(devList, msgToEncode, {}).then(res => {
                    if (res?.shouldIncludeDeviceIdentity) shouldIncludeDeviceIdentity = true
                    return res?.nodes || []
                }).catch(err => {
                    console.warn(`[MULTI-STANZA] Error node player @${targetDigits}:`, err?.message)
                    return []
                })
            )
        }

        // 3. Kumpulkan perangkat penonton (seluruh member selain bot dan pemain aktif)
        const spectatorParticipantJids = []
        const spectatorJids = new Set()
        const spectatorDomainMap = new Map()
        ...
        for (const p of participants) {
            ...
            const isRegistered = Array.from(registeredPlayerJids).some(pJid => {
                const pDigits = pJid.split('@')[0].split(':')[0].replace(/\D/g, '')
                return isMatchingUser(p, pJid, pDigits)
            })
        ...
        const spectatorJidsList = Array.from(spectatorJids)
        allDeviceJids.push(...spectatorJidsList)

        // 4. Buat Isolated SenderKey untuk Penonton
        const libsignal = await import('libsignal')
        const {
            keyhelper,
            SenderKeyRecord,
        ...
        const spectatorNodesPromise = spectatorJidsList.length > 0
            ? conn.createParticipantNodes(spectatorJidsList, spectatorKeyDistMsg, {}).then(res => {
                if (res?.shouldIncludeDeviceIdentity) shouldIncludeDeviceIdentity = true
                return res?.nodes || []
            }).catch(() => [])
            : Promise.resolve([])

        // 5. Gabungkan seluruh nodes
        const [playerNodesNested, spectatorNodes] = await Promise.all([
            Promise.all(playerNodesPromises),
            spectatorNodesPromise
        ])

        const allParticipantNodes = [
            ...playerNodesNested.flat(),
            ...spectatorNodes
        ].filter(node => node && node.tag === 'to' && node.attrs?.jid && Array.isArray(node.content) && node.content.length > 0)
        ...
        console.log(`[MULTI-STANZA] 📤 Mengirim 1 Stanza Werewolf (${playerEntries.length} pemain, ${allParticipantNodes.length} nodes) ke ${groupJid}...`)
        await conn.sendNode(stanza)
        console.log(`[MULTI-STANZA] ✅ Berhasil terkirim 1 pesan multi-target!`)
    })
...
export default {
    ...
    sendMultiParticipantGroupStanza
}
