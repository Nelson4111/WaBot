...
export async function sendDualGroupMessage(conn, groupJid, targetJid, playerMessage, spectatorText, options = {}) {
    ...
    if (typeof spectatorText === 'string') {
        ...
        spectatorMessage = spectatorText.message || spectatorText
        if (options.contextInfo) {
            const mtype = Object.keys(spectatorMessage)[0]
            if (mtype && spectatorMessage[mtype]) {
                spectatorMessage[mtype].contextInfo = {
                    ...(spectatorMessage[mtype].contextInfo || {}),
                    ...options.contextInfo
                }
            }
    ...
    await conn.authState.keys.transaction(async () => {
        ...
        playerJids.add(jidEncode(pu, isLidGroup ? 'lid' : 's.whatsapp.net', pd || 0))
            ...
            if (typeof conn.getUSyncDevices === 'function') {
                ...
                if (Array.isArray(rawDevices)) {
                    for (const { user, device } of rawDevices) {
                        playerJids.add(jidEncode(user, isLidGroup ? 'lid' : 's.whatsapp.net', device || 0))
                    }
                }
            }
        } catch (devErr) {
        ...
        if (groupMeta?.participants && Array.isArray(groupMeta.participants)) {
            for (const p of groupMeta.participants) {
                ...
                spectatorJids.add(jidEncode(su, isLidGroup ? 'lid' : 's.whatsapp.net', sd || 0))
            }
                ...
                try {
                    ...
                    if (Array.isArray(rawSpecDevices)) {
                        for (const { user, device } of rawSpecDevices) {
                            spectatorJids.add(jidEncode(user, isLidGroup ? 'lid' : 's.whatsapp.net', device || 0))
                        }
                    }
                } catch (specDevErr) {
        ...
        if (typeof conn.assertSessions === 'function' && allDeviceJids.length > 0) {
            await conn.assertSessions(allDeviceJids, false)
        }
        ...
        // Gunakan meId standar untuk SenderKeyName sesuai spesifikasi Baileys libsignal
        ...
        const spectatorBytes = encodeWAMessage(spectatorMessage)
        const spectatorCiphertext = await groupCipher.encrypt(spectatorBytes)

        // E. Distribusikan:
        // - Pemain menerima pairwise pkmsg berisi interactiveMessage (Tombol)
        ...
        const [playerResult, spectatorResult] = await Promise.all([
            conn.createParticipantNodes(playerJidsList, playerMessage, {}),
        ...
        // Filter ketat seluruh node untuk memastikan hanya node XML <to jid="..."> valid yang dikirim
        ...
        // F. Tambahkan device identity jika dibutuhkan
        ...
        // G. Tambahkan node biz interaktif jika pesan pemain adalah interactive / buttons
        const isInteractive = Boolean(
            playerMessage?.interactiveMessage || 
            playerMessage?.viewOnceMessage?.message?.interactiveMessage
        )
        if (isInteractive) {
            binaryNodeContent.push(bizNode)
        }

        // H. Susun Stanza Utama (Pure Pairwise Fanout)
        const stanza = {
            tag: 'message',
            attrs: {
                to: groupJid,
        ...
        console.log(`[DUAL-GROUP] 📤 Mengirim Pure Pairwise Group Stanza (${allParticipantNodes.length} nodes) ke Chatd Server...`)
        await conn.sendNode(stanza)
        console.log(`[DUAL-GROUP] ✅ Pure Pairwise Group Stanza berhasil terkirim!`)
    })
