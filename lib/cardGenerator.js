...
export async function generateFishingCard({ avatarUrl, username, ikan, exp, rodLevel = 0 }) {
    ...
    const raw = String(ikan || '')

    const fishIcon = raw.match(/[\u2600-\u27BF\u{1F000}-\u{1FAFF}]/u)?.[0] || '🐟'
    const clean = raw.replace(/[\u2600-\u27BF\u{1F000}-\u{1FAFF}]/gu, '')
        .replace(/_/g, ' ')
        .replace(/[^a-zA-Z0-9\s-]+/g, '')
        .trim()

    const fishLabel = clean.split(/\s+/).filter(Boolean).slice(0, 1).join(' ')
    const fishNameForCard = fishLabel.toUpperCase() || 'FISH'
    ...
    const svg = `<svg width="${CARD_W}" height="${CARD_H}" xmlns="http://www.w3.org/2000/svg">
        ...
        <text x="120" y="${panelY + 84}" font-family="Arial Black,sans-serif"
              font-size="20" fill="#FFD700">${fishIcon} 1 ${esc(fishNameForCard, 12)}</text>
